...
*** Implement DSDE.
    - The 1998 Barros paper seems like the simplest solution.
    - I think we can also learn a few things from it:
      - How to implement a root coordinator.
        - Runs until the model becomes passive.
      - Maybe we can do constructor/destructor.
      - Ignore messages if time is not right.
        - This might be something we messed up in the parallel coordinator,
          because I thought the book was wrong.
...
** Determine how to to implement a sufficient root coordinator
   - Refer to the Time Warp example.

   - I think we can implement an optimistic system that is based on our
     knowledge that Viewer3D doesn't send messages.
   ...
   - I think we should implement Time Warp, but not now.
