...
** On message simultaneity
   ...
   Note that at least one of the papers says that the system preserves sender
   message order, but we aren't doing that. I don't understand why we would want
   that and supporting it would complicate the implementation. For example, we
   couldn't use maps to represent message bags, because the order of message
...
** On two types of graphs
   ...
   1. The model hierarchy, where every model is either an atomic model or a
      network model that is composed of atomic models and deeper network
      models.
   ...
   network may connect to another model. That result is that messages flow from
...
** On efficient organization of mixed real-time and non-real-time systems
   ...
   I assume that the overwhelming majority of real-time systems will include
   ...
   whatever bridges the root of that branch to the parent real-time simulator.
...
** On network structure changes
   ...
   Network structure changes are communicated via the normal DEVS "in model"
   message system.

   Alternatively, they could be implemented as a system-level message (like the
   state transition functions) and incorporated into the simulation update
   algorithm. That's how they are typically described in the literature.

   I chose to implement the network structure changes as normal messages because
   it seemed much simpler to implement, model, and comprehend.
...
** On real-time simulators
   ...
   A fixed time step can be used to simplify real-time implementations. It
   especially helps handle a human-in-the-loop. Note that this makes the
   top-level of the simulator like a traditional game architecture.
   ...
   - Handling no-op transitions is sub-optimal and it is not obvious why it is
     needed.
