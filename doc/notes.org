...
** Implement a RT root coordinator.
   ...
   - http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.843.6421&rep=rep1&type=pdf
     - Section 4.1: How to avoid accumulating delays due to actual execution
       time.
       - See last paragraph.
** Implement flattening optimization.
...
** Create new version that uses one model.
*** First, transform this new model into the standard models.
*** Then, if that works, write a simpler implementation just to support this new model.
