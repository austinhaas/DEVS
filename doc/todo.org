* TODO
** DONE Create a mixed RT and non-RT network.
   - Just use generator and delay.
   - Determine how to integrate the simulator adapter.
** Rename model->sim to find-simulator.
...
** Create a rt simple dynamic test.
** Add docstrings to the rt-models.
** Add more tests.
*** DONE Test atomic-simulator exceptions
*** DONE Test network-simulator exceptions
*** DONE Test rt-atomic-simulator exceptions
*** DONE Test rt-network-simulator exceptions
*** DONE Test rt-step-root-coordinator by faking the wall-time updates.
*** DONE Test changing RT clock scale-factor
*** DONE Determine how to use rt-afap-root-coordinator.
    - We want to use all of the same tests for non-rt and rt implementations.

    - The easiest solution is probably to just add the extra value in the
...
*** Test atomic-model/->RT confluent-update
