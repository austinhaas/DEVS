(ns pettomato.devs.models.network-model-test
  (:require
   ...
   [pettomato.devs.examples.models :refer [delay1]]
...
(deftest invalid-network-models
  ...
  (is (thrown-with-msg? #?(:clj clojure.lang.ExceptionInfo
                        ...
                        (network-model {:x (delay1 5)} [[:network :in :y :in]])))

  (is (thrown-with-msg? #?(:clj clojure.lang.ExceptionInfo
                           :cljs ExceptionInfo)
                        #"A model cannot use the same port for both input and output."
                        (network-model {:x (delay1 5)} [[:network :in :x :in]
                                                        [:x :in :network :out]])))

  (is (thrown-with-msg? #?(:clj clojure.lang.ExceptionInfo
                           :cljs ExceptionInfo)
                        #"A network input port cannot connect directly to a network output port"
                        (network-model {:x (delay1 5)} [[:network :in :network :out]]))))
